<template>
    <div class="cet6-container">
        <!-- 章节选择对话框 -->
        <ChapterSelectDialog v-model="showChapterDialog" level="cet6" @select="handleChapterSelect" />
        ...
        <div v-if="mode === 'exam'" class="exam-mode">
            <div v-if="currentExamWord" class="exam-card">
                    ...
                    <div v-if="showExamFeedback" class="exam-feedback">
                        <n-alert :type="isExamCorrect ? 'success' : 'error'" :title="isExamCorrect ? '回答正确！' : '回答错误'">
                            <template v-if="!isExamCorrect">
                                <div v-if="!showAnswerHint">
                                    <p>答案不正确，请使用退格键修改后继续</p>
                                    <n-button size="small" type="info" @click="showAnswerHint = true" style="margin-top: 8px;">
                                        查看答案
                                    </n-button>
                                </div>
                                <div v-else>
                ...
                <div class="info-box">
                    <div class="info-item">
                        <span class="info-label">检验进度</span>
                    ...
                    <n-button class="end-exam-button" @click="endExam">
                        结束检验
                    </n-button>
                </div>
            </div>
...
const examResults = ref<Array<{ wordId: number | string; isMastered: number }>>([])
const userInputArray = ref<string[]>([])
const showExamFeedback = ref(false)
const isExamCorrect = ref(false)
const showAnswerHint = ref(false) // 是否显示答案提示
...
const examProgressText = computed(() => {
    const correctCount = examResults.value.filter(r => r.isMastered === 1).length
    return `${correctCount} / ${examWords.value.length}`
})
...
const handleExamKeyDown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'Backspace') {
        ...
        // 检查是否填满
        const isFull = userInputArray.value.every(char => char !== '')
        if (isFull) {
            checkExamAnswer()
        }
    }
